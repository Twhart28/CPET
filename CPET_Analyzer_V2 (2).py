...
def LegendMenu(component_id: str, options: list[tuple[str, str]], value=None):
    """
    component_id: e.g., 'legend-vslope' or 'legend-vo2'
    options: [(value, label), ...]  e.g., [('slope','Slope lines (green)'), ('vt1','VT1')]
    """
    if value is None:
        default_vals = [v for v, _ in options]
    else:
        selected = set(value)
        default_vals = [v for v, _ in options if v in selected]
    ...
    return dmc.Popover(
        ...
        children=[
            dmc.PopoverTarget(
                dmc.Button("Legend", size="sm", variant="light", color="gray")
            ),
            dmc.PopoverDropdown(
                dmc.CheckboxGroup(id=component_id, value=default_vals, children=checks)
            ),
        ],
    )
...
def threshold_detection_table(vt_source=None, highlight=None):
    ...
    for tl, tr in zip(tests_left, tests_right):
        ...
        row_active = row_active_vt1 or row_active_vt2
        ...
        row_style = {"boxShadow": "inset 0 0 0 2px #1E5F96"} if row_active else {}
        body_rows.append(html.Tr([left] + vt1_cells + vt2_cells, style=row_style))
                            ...
                            dmc.Group(
                                [
                                    ...
                                    dmc.Group(
                                        [
                                            html.Div(
                                                id="vt-edit-legend-container",
                                                children=LegendMenu(
                                                    "vt-edit-legend",
                                                    EDIT_LEGEND_OPTIONS.get("vslope", []),
                                                ),
                                            ),
            ...
            html.Script(
                ...
                (function(){
                    ...
                    document.addEventListener('keydown', function(ev){
                        ...
                        const key = ev.key || '';
                        const isUndo = key.toLowerCase() === 'z' && !ev.shiftKey;
                        const isRedo = key.toLowerCase() === 'y' || (key.toLowerCase() === 'z' && ev.shiftKey);
                        if (isUndo && !undoBtn.disabled) {
                            undoBtn.click();
                            ev.preventDefault();
                        } else if (isRedo && redoBtn && !redoBtn.disabled) {
                            redoBtn.click();
                            ev.preventDefault();
                        }
                    });
...
def compute_vt_context(analysis: dict, overrides: dict | None = None):
    ...
    vt1_rows = {
        ...
        "VE/VO₂ , VE/CO₂": _mk_row_from_time(vt_veeq.get("vt1"), t, vo2_L, rer_L, vo2_peak),
...
def _init_edit_state(row_id: int, analysis: dict, context: dict):
    ...
    graphs = {
        "vslope": {
            ...
            "legend": list(EDIT_LEGEND_DEFAULTS.get("vslope", [])),
            "view": {},
        },
        "veeq": {
            "current": {
            ...
            "legend": list(EDIT_LEGEND_DEFAULTS.get("veeq", [])),
            "view": {},
        },
        "pet": {
            "current": {
            ...
            "legend": list(EDIT_LEGEND_DEFAULTS.get("pet", [])),
            "view": {},
        },
    }
...
def _build_edit_figure(graph_key: str, analysis_payload: dict, graph_state: dict):
    ...
    fig = go.Figure()
    meta = {"segments": [], "vt1": None, "vt2": None, "ranges": {}}

    legend_values = _normalize_edit_legend(graph_key, graph_state.get("legend"))
    graph_state["legend"] = legend_values
    legend_set = set(legend_values)
    view = graph_state.setdefault("view", {})
    ...
    if graph_key == "vslope":
        vo2 = analysis["vo2"]
        vco2 = analysis["vco2"]
        segments_payload = graph_state.get("current", {}).get("segments")
        segments = _segments_from_payload(segments_payload)
            ...
            for ((x0, y0), (x1, y1)) in segments:
                shapes.append(
                    dict(
                        ...
                        x1=x1,
                        y1=y1,
                        line=dict(color=COL_GREEN, width=3),
                    )
                )
                meta["segments"].append(shape_idx)
                shape_idx += 1
        ...
        x_vals = list(vo2[np.isfinite(vo2)])
        y_vals = list(vco2[np.isfinite(vco2)])
        for ((x0, y0), (x1, y1)) in segments:
            x_vals.extend([x0, x1])
            y_vals.extend([y0, y1])

        allowed_ranges["x"] = _calc_axis_range(x_vals)
        allowed_ranges["y"] = _calc_axis_range(y_vals)
        ...
        fig.add_trace(
            go.Scatter(
                ...
                name="VE/VO₂ (points)",
                legendgroup="vevo2_pts",
                visible="vevo2_pts" in legend_set,
                marker=dict(size=6, opacity=0.75, color=COL_BLUE),
                showlegend=False,
            )
        )
        fig.add_trace(
            go.Scatter(
                ...
                name="VE/VO₂ (trend)",
                legendgroup="vevo2_tr",
                visible="vevo2_tr" in legend_set,
                line=dict(color=COL_BLUE_DARK, width=2),
                showlegend=False,
            )
        )
        fig.add_trace(
            go.Scatter(
                ...
                name="VE/VCO₂ (points)",
                legendgroup="vevco2_pts",
                visible="vevco2_pts" in legend_set,
                marker=dict(size=6, opacity=0.75, color=COL_ORANGE),
                showlegend=False,
            )
        )
        fig.add_trace(
            go.Scatter(
                ...
                name="VE/VCO₂ (trend)",
                legendgroup="vevco2_tr",
                visible="vevco2_tr" in legend_set,
                line=dict(color=COL_ORANGE_DARK, width=2),
                showlegend=False,
            )
        )
        ...
        y_vals = np.concatenate([ve_vo2, ve_vco2, ve_vo2_L, ve_vco2_L])
        allowed_ranges["x"] = _calc_axis_range(t)
        allowed_ranges["y"] = _calc_axis_range(y_vals)
        ...
        fig.add_trace(
            go.Scatter(
                ...
                name="PetO₂ (points)",
                legendgroup="peto2_pts",
                visible="peto2_pts" in legend_set,
                marker=dict(size=6, opacity=0.75, color=COL_BLUE),
                showlegend=False,
            )
        )
        fig.add_trace(
            go.Scatter(
                ...
                name="PetO₂ (trend)",
                legendgroup="peto2_tr",
                visible="peto2_tr" in legend_set,
                line=dict(color=COL_BLUE_DARK, width=2),
                showlegend=False,
            )
        )
        fig.add_trace(
            go.Scatter(
                ...
                name="PetCO₂ (points)",
                legendgroup="petco2_pts",
                visible="petco2_pts" in legend_set,
                marker=dict(size=6, opacity=0.75, color=COL_ORANGE),
                showlegend=False,
            )
        )
        fig.add_trace(
            go.Scatter(
                ...
                name="PetCO₂ (trend)",
                legendgroup="petco2_tr",
                visible="petco2_tr" in legend_set,
                line=dict(color=COL_ORANGE_DARK, width=2),
                showlegend=False,
            )
        )
        ...
        y_vals = np.concatenate([peto2n, petco2n, peto2n_L, petco2n_L])
        allowed_ranges["x"] = _calc_axis_range(t)
        allowed_ranges["y"] = _calc_axis_range(y_vals)
    ...
    # apply axis ranges and clamp zoom-out to allowed ranges
    x_allowed = allowed_ranges.get("x") or [-1.0, 1.0]
    y_allowed = allowed_ranges.get("y") or [-1.0, 1.0]
    meta["ranges"] = {"x": x_allowed, "y": y_allowed}

    current_view_x = view.get("x_range")
    if not current_view_x:
        current_view_x = x_allowed
    else:
        current_view_x = _clamp_range(current_view_x, x_allowed)
    view["x_range"] = current_view_x

    current_view_y = view.get("y_range")
    if not current_view_y:
        current_view_y = y_allowed
    else:
        current_view_y = _clamp_range(current_view_y, y_allowed)
    view["y_range"] = current_view_y

    fig.update_xaxes(range=current_view_x)
    fig.update_yaxes(range=current_view_y)
...
def _render_edit_legend(graph_key: str, graph_state: dict):
    options = EDIT_LEGEND_OPTIONS.get(graph_key, [])
    selection = _normalize_edit_legend(graph_key, (graph_state or {}).get("legend"))
    return LegendMenu("vt-edit-legend", options, value=selection)
...
EDIT_LEGEND_DEFAULTS = {
    key: [val for val, _ in opts]
    for key, opts in EDIT_LEGEND_OPTIONS.items()
}


def _normalize_edit_legend(graph_key: str, values):
    defaults = EDIT_LEGEND_DEFAULTS.get(graph_key, [])
    if not values:
        return list(defaults)
    allowed = {v for v, _ in EDIT_LEGEND_OPTIONS.get(graph_key, [])}
    normalized = [v for v in defaults if v in values and v in allowed]
    # include any additional (unexpected) values in original order to avoid losing state
    normalized.extend([v for v in values if v in allowed and v not in normalized])
    return normalized or list(defaults)


def _calc_axis_range(values, pad: float = 0.05):
...
def _clamp_range(range_vals, allowed):
    if not allowed or len(allowed) != 2:
        return list(range_vals)
    lo, hi = float(allowed[0]), float(allowed[1])
    if lo > hi:
        lo, hi = hi, lo
    r0, r1 = float(range_vals[0]), float(range_vals[1])
    rlo, rhi = min(r0, r1), max(r0, r1)
    rlo = max(lo, rlo)
    rhi = min(hi, rhi)
    if rhi <= rlo:
        epsilon = max((hi - lo) * 1e-6, 1e-6)
        mid = (rlo + rhi) / 2.0
        rlo = max(lo, mid - epsilon)
        rhi = min(hi, mid + epsilon)
    return [float(rlo), float(rhi)] if r0 <= r1 else [float(rhi), float(rlo)]


def _get_axis_update(relayout: dict, axis_key: str):
    if not relayout:
        return None
    key_range = f"{axis_key}.range"
    r0 = relayout.get(f"{key_range}[0]")
    r1 = relayout.get(f"{key_range}[1]")
    if r0 is not None and r1 is not None:
        try:
            return float(r0), float(r1)
        except Exception:
            return None
    if key_range in relayout:
        val = relayout.get(key_range)
        if isinstance(val, (list, tuple)) and len(val) == 2:
            try:
                return float(val[0]), float(val[1])
            except Exception:
                return None
    auto_key = f"{axis_key}.autorange"
    if relayout.get(auto_key):
        return "auto"
    return None
...
def _apply_relayout(state: dict, graph_key: str, relayout: dict):
    ...
    for key, val in relayout.items():
        ...
        idx = int(m.group(1))
        attr = m.group(2)
        shape_updates.setdefault(idx, {})[attr] = val

    x_update = _get_axis_update(relayout, "xaxis")
    y_update = _get_axis_update(relayout, "yaxis")

    if not shape_updates and x_update is None and y_update is None:
        return state, False
    ...
    if shape_changed:
        ...
        g.setdefault("undo", []).append(original_current)
        g["redo"] = []
        g["current"] = current

    view_changed = False
    allowed = meta.get("ranges") or {}
    view = dict(g.get("view") or {})

    if x_update is not None:
        if x_update == "auto":
            new_range = allowed.get("x")
        else:
            new_range = _clamp_range(x_update, allowed.get("x")) if allowed.get("x") else list(x_update)
        if new_range and view.get("x_range") != list(new_range):
            view["x_range"] = list(new_range)
            view_changed = True

    if y_update is not None:
        if y_update == "auto":
            new_range = allowed.get("y")
        else:
            new_range = _clamp_range(y_update, allowed.get("y")) if allowed.get("y") else list(y_update)
        if new_range and view.get("y_range") != list(new_range):
            view["y_range"] = list(new_range)
            view_changed = True

    if view_changed:
        g["view"] = view

    if not shape_changed and not view_changed:
        return state, False
...
def make_directory_app():
    ...
    @app.callback(
        ...
        Output("vt-edit-legend-container", "children"),
    ...
    def _handle_vt_edit(
        ...
        if triggered == "vt-edit-close":
            return False, None, dash.no_update, dash.no_update, True, True, True, dash.no_update, dash.no_update, dash.no_update
        ...
        if triggered == "btn-edit-vt":
            ...
            legend_component = _render_edit_legend(active, state["graphs"].get(active))
            return (
                True,
                state,
                fig,
        ...
        if triggered == "vt-edit-legend":
            legend_norm = _normalize_edit_legend(active, legend_value)
            current_legend = state["graphs"].setdefault(active, {}).get("legend")
            if legend_norm != current_legend:
                state["graphs"][active]["legend"] = legend_norm
                needs_refresh = True
        ...
        legend_component = _render_edit_legend(active, graph_state)
