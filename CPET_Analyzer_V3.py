...
def LegendMenu(component_id: str, options: list[tuple[str, str]], value: list[str] | None = None):
    """
    ...
    return dmc.Popover(
        ...
        children=[
            ...
            dmc.PopoverDropdown(dmc.CheckboxGroup(id=component_id, value=value, children=checks)),
        ],
    )

VT_EDIT_LEGEND_OPTIONS: dict[str, list[tuple[str, str]]] = {
    "vslope": [
                            ...
                            dmc.Group(
                                [
                                    ...
                                    dmc.Group(
                                        [
                                            dmc.Box(
                                                LegendMenu("vt-edit-legend", VT_EDIT_LEGEND_OPTIONS["vslope"]),
...
def _init_edit_state(row_id: int, analysis: dict, context: dict):
    ...
    legend_defaults = {
        key: [opt for opt, _ in VT_EDIT_LEGEND_OPTIONS.get(key, [])]
        for key in VT_EDIT_LEGEND_OPTIONS
    }
...
def _build_edit_legend_component(graph_key: str, selected: list[str] | None):
    options = VT_EDIT_LEGEND_OPTIONS.get(graph_key, [])
    if selected is None:
        selected = [opt for opt, _ in options]
    return LegendMenu("vt-edit-legend", options, value=selected)


def _build_edit_figure(
    graph_key: str,
    ...
    legend_vals = legend_values
    if legend_vals is None:
        legend_vals = [opt for opt, _ in VT_EDIT_LEGEND_OPTIONS.get(graph_key, [])]
    legend_set = set(legend_vals or [])

    if graph_key == "vslope":
        vo2 = analysis["vo2"]
        vco2 = analysis["vco2"]
        if "points" in legend_set:
            fig.add_trace(
                go.Scatter(
                    x=vo2,
                    y=vco2,
...
def make_directory_app():
    ...
    def _handle_vt_edit(
        ...
        legend_children = dash.no_update
        ...
        if triggered == "btn-edit-vt":
            ...
            active = state["active"]
            legend_state = state.get("legend") or {}
            legend_values_active = legend_state.get(active)
            fig, meta = _build_edit_figure(
                active,
                state["analysis"],
                state["graphs"][active],
                legend_values_active,
            )
            ...
            legend_children = _build_edit_legend_component(active, legend_values_active)
            return (
                True,
                state,
                fig,
        ...
        legend_state = state.setdefault("legend", {})
        needs_refresh = False
        ...
        if triggered in ("vt-edit-prev", "vt-edit-next"):
            ...
            active = state["active"]
            legend_state.setdefault(active, [opt for opt, _ in VT_EDIT_LEGEND_OPTIONS.get(active, [])])
            legend_children = _build_edit_legend_component(active, legend_state.get(active))
            needs_refresh = True
        ...
        if triggered == "vt-edit-legend":
            new_vals = list(legend_value or [])
            current_vals = list(legend_state.get(active) or [])
            if new_vals != current_vals:
                legend_state[active] = new_vals
                needs_refresh = True
