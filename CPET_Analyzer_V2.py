...
# ---- EventListener shim (must NOT import components inside callbacks) ----
try:
    from dash_extensions import EventListener as _EventListener  # real component (has `events`, `n_events`)
except Exception:
    # Fallback: behave like a no-op Div and silently ignore `events` kwarg
    from dash import html
    class _EventListener(html.Div):
        def __init__(self, *a, **k):
            k.pop("events", None)          # drop unsupported prop
            k.setdefault("children", [])
            super().__init__(*a, **k)

EventListener = _EventListener  # use this everywhere

# ---------- persistent VT overrides ----------
APP_STATE_DIR = Path.home() / ".cpet_analyzer"
OVERRIDES_DIR = APP_STATE_DIR / "vt_overrides"
OVERRIDES_DIR.mkdir(parents=True, exist_ok=True)

def _ovr_key(p: str) -> str:
    # stable key by absolute path (handles duplicates across machines)
    return hashlib.sha1(str(Path(p).resolve()).encode("utf-8")).hexdigest()

def load_overrides(path: str) -> dict:
    f = OVERRIDES_DIR / (_ovr_key(path) + ".json")
    if f.exists():
...
def VTEditorModal():
    # Hotkeys via EventListener (defined at module level above)
    events = [{"event": "keydown", "props": ["key", "ctrlKey"]}]

    return html.Div([
        # state
        dcc.Store(id="vt-ed-active"),          # "vslope" | "veeq" | "pet"
        dcc.Store(id="vt-ed-state"),           # editor state
        dcc.Store(id="vt-ed-hist"),
        dcc.Store(id="vt-ed-idx"),
        dcc.Store(id="vt-ed-dirty"),
        dcc.Store(id="vt-ed-file"),
        dcc.Store(id="vt-ed-refresh"),
        # hotkeys
        EventListener(id="vt-ed-hotkeys", events=events),
        # modal (unchanged...)
        dmc.Modal(
            id="vt-ed-modal", centered=True, size="88%",
            withCloseButton=False, overlayProps={"opacity": 0.45, "blur": 2},
            children=dmc.Stack(gap="sm", children=[
                # toolbar row
                dmc.Group(align="center", justify="space-between", children=[
                    dmc.Group(gap="md", children=[
                        dmc.Button("Undo", id="vt-ed-undo", variant="filled", color="blue"),
                        dmc.Button("Redo", id="vt-ed-redo", variant="filled", color="blue"),
                        dmc.Button("Save Chart", id="vt-ed-save", variant="filled", color="teal"),
                    ]),
                    dmc.Button("X", id="vt-ed-close", color="red", variant="filled"),
                ]),
                # graph pager controls + figure
                dmc.Group(align="center", justify="space-between", children=[
                    dmc.ActionIcon("←", id="vt-ed-prev", variant="filled"),
                    dcc.Graph(
                        id="vt-ed-graph",
                        config={"editable": True, "edits": {"shapePosition": True}},
                        style={"height": "640px"}
                    ),
                    dmc.ActionIcon("→", id="vt-ed-next", variant="filled"),
                ]),
                # inline threshold table mirroring dashboard (we'll highlight active row in callback)
                dmc.Paper(id="vt-ed-threshold", withBorder=True, radius="xl", p="sm"),
            ])
        )
    ])

# ------- Figure builders for the editor (use shapes so autoscale ignores VT heights) -------
def _build_vslope_editor(figs_payload: dict, vt_state: dict):
    import plotly.graph_objects as go
    # points
    base = figs_payload["vslope_points"]  # (x,y) arrays
    xs, ys = base["x"], base["y"]
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=xs, y=ys, mode="markers",
                             name="VCO₂–VO₂ points", marker=dict(size=6, opacity=0.75)))
    # green segments as draggable shapes
    shapes = []
    for i, seg in enumerate(vt_state.get("vslope_segs", [])):
        (x1, y1), (x2, y2) = seg
        shapes.append(dict(type="line", x0=x1, y0=y1, x1=x2, y1=y2,
                           line=dict(width=3, color="#008C46")))
    # VT verticals in VO2 space; lock Y via yref='paper' -> ignores autoscale
    for key, col in [("vt1_vo2", "#800080"), ("vt2_vo2", "#FF0000")]:
        x = vt_state.get(key, None)
        if x is not None and isinstance(x, (int, float)):
            shapes.append(dict(type="line", x0=x, x1=x, y0=0, y1=1,
                               xref="x", yref="paper",
                               line=dict(width=2, dash="dot", color=col)))
    fig.update_layout(shapes=shapes, margin=dict(l=8, r=8, t=24, b=8),
                      xaxis_title="VO₂ (L·min⁻¹)", yaxis_title="VCO₂ (L·min⁻¹)")
    return fig

def _build_time_editor(figs_payload: dict, vt_state: dict, panel: str):
    import plotly.graph_objects as go
    # choose which time series bundle to draw
    data = figs_payload[panel]  # dict of traces {"pts":[(x,y),...], "loess":[(x,y),...]}
    fig = go.Figure()
    # points
    for name, xy in data.get("pts", {}).items():
        fig.add_trace(go.Scatter(x=xy["x"], y=xy["y"], mode="markers", name=name,
                                 marker=dict(size=6, opacity=0.75)))
    # loess lines
    for name, xy in data.get("loess", {}).items():
        fig.add_trace(go.Scatter(x=xy["x"], y=xy["y"], mode="lines", name=f"{name} Trace"))
    # VT lines (time); lock Y via paper so they don't affect autoscale
    shapes = []
    for key, col in [("vt1_t", "#800080"), ("vt2_t", "#FF0000")]:
        x = vt_state.get(key, None)
        if x is not None and isinstance(x, (int, float)):
            shapes.append(dict(type="line", x0=x, x1=x, y0=0, y1=1,
                               xref="x", yref="paper", line=dict(width=2, dash="dot", color=col)))
    ylab = "Ratio" if panel == "veeq" else "Normalized (÷ series mean)"
    title = {"veeq": "VE/VO₂ & VE/VCO₂ vs Time", "pet": "PetO₂ & PetCO₂ (normalized) vs Time"}[panel]
    fig.update_layout(shapes=shapes, margin=dict(l=8, r=8, t=24, b=8),
                      xaxis_title="Time (min)", yaxis_title=ylab, title=title)
    return fig

def _intersect(p, q):
    # lines given by ((x1,y1),(x2,y2)); return x of intersection (None if parallel)
...
def register_vt_editor_callbacks(app):
    import plotly.graph_objects as go

    # 1) Open editor: build initial state from current rendered file
    @app.callback(
        Output("vt-ed-modal","opened"),
        Output("vt-ed-file","data"),
        Output("vt-ed-active","data"),
        Output("vt-ed-state","data"),
        Output("vt-ed-hist","data"),
        Output("vt-ed-idx","data"),
        Output("vt-ed-dirty","data"),
        Output("vt-ed-graph","figure"),
        Output("vt-ed-threshold","children"),
        Input("btn-edit-vt","n_clicks"),
        State("selected-row-id","data"),
        prevent_initial_call=True
    )
    def _open_editor(n, rid):
        if not (n and rid): raise dash.exceptions.PreventUpdate
        row = get_participant(int(rid)) or {}
        path = (row.get("file_path") or "").strip()
        figs, vt_table = build_figures_from_file(path)

        # payload for editor figure builders (raw x/y the editor needs)
        payload = {
            "vslope_points": {"x": figs["vslope"].data[0].x, "y": figs["vslope"].data[0].y},
            "veeq": {
                "pts": {
                    "VE/VO₂":  {"x": figs["veeq"].data[0].x, "y": figs["veeq"].data[0].y},
                    "VE/VCO₂": {"x": figs["veeq"].data[2].x, "y": figs["veeq"].data[2].y},
                },
                "loess": {
                    "VE/VO₂":  {"x": figs["veeq"].data[1].x, "y": figs["veeq"].data[1].y},
                    "VE/VCO₂": {"x": figs["veeq"].data[3].x, "y": figs["veeq"].data[3].y},
                },
            },
            "pet": {
                "pts": {
                    "PetO₂":  {"x": figs["pet"].data[0].x, "y": figs["pet"].data[0].y},
                    "PetCO₂": {"x": figs["pet"].data[2].x, "y": figs["pet"].data[2].y},
                },
                "loess": {
                    "PetO₂":  {"x": figs["pet"].data[1].x, "y": figs["pet"].data[1].y},
                    "PetCO₂": {"x": figs["pet"].data[3].x, "y": figs["pet"].data[3].y},
                },
            },
        }

        # bootstrap editor VT values using whatever is currently drawn
        def _get_vt_from(fig, name):
            for tr in fig.data:
                if tr.name == name and len(tr.x)==2 and tr.x[0]==tr.x[1]:
                    return float(tr.x[0])
            return None

        vt_state = {
            "vt1_vo2": _get_vt_from(figs["vslope"], "VT1"),
            "vt2_vo2": _get_vt_from(figs["vslope"], "VT2"),
            "vt1_t":   _get_vt_from(figs["veeq"], "VT1"),   # time-based (veeq)
            "vt2_t":   _get_vt_from(figs["veeq"], "VT2"),
            "vslope_segs": []  # optional: we’ll fill when user nudges them
        }

        # figure for first page (V-Slope)
        fig = _build_vslope_editor(payload, vt_state)
        hist = [vt_state]
        return True, path, "vslope", vt_state, hist, 0, False, fig, threshold_detection_table(dict(vt_table))

    # 2) Enable/disable buttons & handle hotkeys
    @app.callback(
        Output("vt-ed-undo","disabled"), Output("vt-ed-redo","disabled"), Output("vt-ed-save","disabled"),
        Input("vt-ed-hist","data"), Input("vt-ed-idx","data"), Input("vt-ed-dirty","data")
    )
    def _btn_states(hist, idx, dirty):
        hist = hist or []; idx = idx or 0; dirty = bool(dirty)
        undo_dis = (idx <= 0)
        redo_dis = (idx >= max(len(hist)-1, 0))
        save_dis = (not dirty)
        return undo_dis, redo_dis, save_dis

    # Ctrl+Z / Ctrl+Y via dash-extensions EventListener (works when modal is open)
    app.clientside_callback(
        """
        function(ev, isOpen, idx, hist_len) {
            if(!isOpen || !ev) return window.dash_clientside.no_update;
            if(ev.ctrlKey && ev.key==='z') return {op:'undo'};
            if(ev.ctrlKey && ev.key==='y') return {op:'redo'};
            return window.dash_clientside.no_update;
        }
        """,
        Output("vt-ed-refresh","data"),
        Input("vt-ed-hotkeys","n_events"),
        State("vt-ed-modal","opened"),
        State("vt-ed-idx","data"),
        State("vt-ed-hist","data")
    )

    # 3) Page left/right
    @app.callback(
        Output("vt-ed-active","data"),
        Input("vt-ed-prev","n_clicks"), Input("vt-ed-next","n_clicks"),
        State("vt-ed-active","data"),
        prevent_initial_call=True
    )
    def _page(prev, nxt, active):
        order = ["vslope","veeq","pet"]
        i = order.index(active or "vslope")
        if dash.ctx.triggered_id == "vt-ed-prev": i = (i - 1) % 3
        else:                                      i = (i + 1) % 3
        return order[i]

    # 4) Redraw figure when page or state changes
    @app.callback(
        Output("vt-ed-graph","figure"),
        Input("vt-ed-active","data"), Input("vt-ed-state","data"),
        State("vt-ed-file","data"), prevent_initial_call=True
    )
    def _redraw(active, state, path):
        figs, _ = build_figures_from_file(path)
        payload = {
            "vslope_points": {"x": figs["vslope"].data[0].x, "y": figs["vslope"].data[0].y},
            "veeq": {
                "pts": {"VE/VO₂": {"x": figs["veeq"].data[0].x, "y": figs["veeq"].data[0].y},
                        "VE/VCO₂":{"x": figs["veeq"].data[2].x, "y": figs["veeq"].data[2].y}},
                "loess":{"VE/VO₂": {"x": figs["veeq"].data[1].x, "y": figs["veeq"].data[1].y},
                         "VE/VCO₂":{"x": figs["veeq"].data[3].x, "y": figs["veeq"].data[3].y}},
            },
            "pet": {
                "pts": {"PetO₂": {"x": figs["pet"].data[0].x, "y": figs["pet"].data[0].y},
                        "PetCO₂":{"x": figs["pet"].data[2].x, "y": figs["pet"].data[2].y}},
                "loess":{"PetO₂": {"x": figs["pet"].data[1].x, "y": figs["pet"].data[1].y},
                         "PetCO₂":{"x": figs["pet"].data[3].x, "y": figs["pet"].data[3].y}},
            }
        }
        active = active or "vslope"
        if active == "vslope": return _build_vslope_editor(payload, state or {})
        return _build_time_editor(payload, state or {}, active)

    # 5) Capture drags via relayoutData (shapes move). Enforce x-only for VT lines.
    @app.callback(
        Output("vt-ed-state","data"), Output("vt-ed-hist","data"),
        Output("vt-ed-idx","data"),  Output("vt-ed-dirty","data"),
        Input("vt-ed-graph","relayoutData"),
        State("vt-ed-state","data"), State("vt-ed-hist","data"),
        State("vt-ed-idx","data"), State("vt-ed-active","data"),
        prevent_initial_call=True
    )
    def _on_drag(layout, state, hist, idx, active):
        if not layout: raise dash.exceptions.PreventUpdate
        state = dict(state or {}); hist = list(hist or [state]); idx = int(idx or 0)

        # decode which shape changed
        # For V-Slope we order shapes as: seg0, seg1, seg2, vt1, vt2
        # For time panels: vt1, vt2 only
        def _read(shape_i, axis):
            k = f"shapes[{shape_i}].{axis}"
            return layout[k] if k in layout else None

        if active == "vslope":
            # update segments if any changed
            changed_any = False
            segs = state.get("vslope_segs") or []
            # if user never moved segments, initialize from figure snapshot
            if not segs:
                # no-op: they appear after first drag; until then we treat only VT lines
                segs = []
            # shapes [3] and [4] are VT lines — clamp Y back to paper (x only)
            for line_i, key in [(3, "vt1_vo2"), (4, "vt2_vo2")]:
                x = _read(line_i, "x0") or _read(line_i, "x1")
                if x is not None:
                    state[key] = float(x); changed_any = True

            # If any of seg0..seg2 endpoints moved, recompute VT1/VT2 by intersections
            # We'll probe shape keys present in layout to detect which seg index moved.
            for i in range(3):
                x0 = _read(i, "x0"); y0 = _read(i, "y0")
                x1 = _read(i, "x1"); y1 = _read(i, "y1")
                if None not in (x0,y0,x1,y1):
                    # ensure segs list big enough
                    while len(segs) <= i: segs.append(((x0,y0),(x1,y1)))
                    segs[i] = ((float(x0), float(y0)), (float(x1), float(y1)))
                    changed_any = True
            if changed_any:
                state["vslope_segs"] = segs
                if len(segs) >= 3:
                    x12 = _intersect(segs[0], segs[1]); 
                    x23 = _intersect(segs[1], segs[2])
                    if x12 is not None: state["vt1_vo2"] = x12
                    if x23 is not None: state["vt2_vo2"] = x23
        else:
            # time panels: shapes [0]=vt1, [1]=vt2 — clamp Y (paper) by ignoring y changes
            for line_i, key in [(0,"vt1_t"), (1,"vt2_t")]:
                x = _read(line_i, "x0") or _read(line_i, "x1")
                if x is not None: state[key] = float(x)

        # push history (truncate redo tail if any)
        hist = hist[:idx+1] + [state]
        idx = len(hist) - 1
        return state, hist, idx, True

    # 6) Undo / Redo (buttons and hotkeys share same path via vt-ed-refresh store)
    @app.callback(
        Output("vt-ed-state","data"), Output("vt-ed-idx","data"), Output("vt-ed-dirty","data"),
        Input("vt-ed-undo","n_clicks"), Input("vt-ed-redo","n_clicks"), Input("vt-ed-refresh","data"),
        State("vt-ed-hist","data"), State("vt-ed-idx","data"), prevent_initial_call=True
    )
    def _undo_redo(_u, _r, hot, hist, idx):
        hist = hist or []; idx = int(idx or 0)
        trig = dash.ctx.triggered_id
        if trig in ("vt-ed-undo",) or (isinstance(hot, dict) and hot.get("op")=="undo"):
            idx = max(0, idx-1)
        elif trig in ("vt-ed-redo",) or (isinstance(hot, dict) and hot.get("op")=="redo"):
            idx = min(len(hist)-1, idx+1)
        return hist[idx], idx, True

    # 7) Save → write JSON + refresh dashboard
    @app.callback(
        Output("vt-ed-modal","opened"), Output("vt-ed-refresh","data"),
        Input("vt-ed-save","n_clicks"),
        State("vt-ed-file","data"), State("vt-ed-state","data"),
        State("vt-ed-active","data"), State("vt-ed-refresh","data"),
        prevent_initial_call=True
    )
    def _save(n, path, state, active, bump):
        if not n: raise dash.exceptions.PreventUpdate
        ovr = load_overrides(path)
        ovr = dict(ovr or {})
        # fold current page values into overrides
        if active == "vslope":
            ovr["vslope"] = {
                "vt1_vo2": state.get("vt1_vo2"),
                "vt2_vo2": state.get("vt2_vo2"),
                "segs": state.get("vslope_segs", [])
            }
        elif active == "veeq":
            v = ovr.get("veeq", {})
            v.update({"vt1_t": state.get("vt1_t"), "vt2_t": state.get("vt2_t")})
            ovr["veeq"] = v
        else:
            v = ovr.get("pet", {})
            v.update({"vt1_t": state.get("vt1_t"), "vt2_t": state.get("vt2_t")})
            ovr["pet"] = v
        save_overrides(path, ovr)
        # close modal and bump a token that the dashboard listens to
        return False, (bump or 0) + 1

    # 8) Close without saving
    @app.callback(Output("vt-ed-modal","opened"),
                  Input("vt-ed-close","n_clicks"), prevent_initial_call=True)
    def _close(_): return False
...
def make_dashboard_app():
    ...
    register_vt_editor_callbacks(app)
    return app
...
def build_figures_from_file(path: str):
    ...
    # ---- apply user overrides (if any) BEFORE building figures ----
    _ovr = load_overrides(path)
    if _ovr:
        # V-Slope edits use VO2-space x for VT lines (and we may also store manual segments)
        o = _ovr.get("vslope", {})
        vt1_vslope_vo2 = float(o.get("vt1_vo2", vt1_vslope_vo2))
        vt2_vslope_vo2 = float(o.get("vt2_vo2", vt2_vslope_vo2))

        # VE/VO2 & VE/VCO2 panel VT times
        o = _ovr.get("veeq", {})
        vt1_vevo2_t  = float(o.get("vt1_t", vt1_vevo2_t))
        vt2_vevco2_t = float(o.get("vt2_t", vt2_vevco2_t))

        # PetO2 & PetCO2 panel VT times
        o = _ovr.get("pet", {})
        vt1_peto2_t  = float(o.get("vt1_t", vt1_peto2_t))
        vt2_petco2_t = float(o.get("vt2_t", vt2_petco2_t))
...
def make_directory_app():
    ...
    def _render_dashboard(route, rid, vslope_opts, vo2_opts, veeq_opts, pet_opts, rer_opts):
        ...
        # Fallback for older DB rows: try CPET_Uploads/<file_name>
        if not path:
            fname = (row.get("file_name") or "").strip()
            if fname:
                candidate = (APP_DIR / "CPET_Uploads" / fname)
                if candidate.exists():
                    path = str(candidate)
